...
options:
    ...
    link_templates:
        ...
        type: list
        elements: str
    unlink_all_templates:
        description:
            - Unlink all linked tempalte from the host
        type: bool
        default: "no"
...
- name: Unlink all linked template
# Set current task level variables for Zabbix Server host in task
  vars:
    ansible_network_os: community.zabbix.zabbix
    ansible_connection: httpapi
...
class Host(ZabbixBase):
    ...
    def link_or_clear_template(self, host_id, template_id_list, unlink_all_templates=False, clear_all_link_templates=False):
        ...
        if unlink_all_templates or clear_all_link_templates:
        ...
        if unlink_all_templates:
            request_str = {"hostid": host_id}
            error_msg = "Failed to unlink all templates from host: %s"
        try:
            if self._module.check_mode:
                self._module.exit_json(changed=True)
...
def main():
    ...
    argument_spec.update(dict(
        ...
        unlink_all_templates=dict(type="bool", default=False),
    ...
    unlink_all_templates = module.params["unlink_all_templates"]
    ...
    template_ids = []
    if link_templates or unlink_all_templates or clear_all_link_templates:
    ...
    if is_host_exist:
        ...
        if state == "absent":
            ...
            if host.check_all_properties(
                ...
                if unlink_all_templates:
                    host.link_or_clear_template(host_id, template_ids,unlink_all_templates=True)
                else:
